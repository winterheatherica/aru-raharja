...
import { ADMIN_SERVICE_MATRIX_URL, SERVICE_SOLUTIONS, ServiceCode, serviceLabel } from "./_shared";
...
export default function ServiceMatrixListPage({ locale, dict }: { locale: Locale; dict?: Dictionary }) {
  const t = (dict as any)?.admin?.serviceMatrix?.list;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setError(null);
      try {
        const res = await fetch(`${ADMIN_SERVICE_MATRIX_URL}/service/${selected.toUpperCase()}?lang=${locale.toUpperCase()}`, { credentials: "include" });
        if (!res.ok) throw new Error(await res.text());
        setItems(await res.json());
      } catch (e: any) {
        setError(e?.message || "Failed to load service matrices");
      } finally {
        setLoading(false);
      }
    })();
  ...
  async function onDelete(id: string) {
    ...
    try {
      const res = await fetch(`${ADMIN_SERVICE_MATRIX_URL}/${id}`, { method: "DELETE", credentials: "include" });
      if (!res.ok) throw new Error(await res.text());
  ...
  return (
    <main className="mx-auto max-w-7xl p-6 grid gap-6 md:pl-72">
      <section className="rounded-2xl border border-bumnslate-10 bg-bumn-gradient-white-4 p-5 shadow-bumn-2">
        <h1 className="text-2xl font-semibold text-bumnblue-2">{t?.title ?? "Admin Service Matrix"}</h1>
        <p className="mt-1 text-sm text-bumnslate-6">{t?.subtitle ?? "Choose a service to manage matrix."}</p>
      </section>
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {SERVICE_SOLUTIONS.map((svc) => (
          <Link
            key={svc}
            href={`/${locale}/admin/service-matrix?service=${svc}`}
            className={[
      ...
      {selected && (
        <section className="grid gap-4">
          <div className="flex items-center justify-between">
            ...
            <Link href={`/${locale}/admin/service-matrix/create?service=${selected}`} className="inline-flex items-center justify-center rounded-xl bg-bumn-gradient-primary-11 px-4 py-2 text-sm font-medium text-white shadow-bumn-2 transition hover:opacity-95">{t?.createButton ?? "Create Matrix"}</Link>
          </div>
              ...
              {items.map((it) => (
                <article key={it.id} className={["rounded-2xl border border-bumnslate-10 bg-bumn-gradient-white-4 p-5 shadow-bumn-2 grid gap-3", it.is_active ? "ring-2 ring-bumnblue-5" : ""].join(" ")}>
                  <div className="text-sm grid gap-1 text-bumnslate-6">
                    <p><b>ID:</b> {it.id}</p>
                    <p><b>Compact:</b> {String(it.compact)}</p>
                    <p><b>Active:</b> {String(it.is_active)}</p>
                    <p><b>Title:</b> {it.title || "-"}</p>
                    {it.description && <p><b>Description:</b> {it.description}</p>}
                    {it.footnote && <p><b>Footnote:</b> {it.footnote}</p>}
                  </div>

                  <div className="pt-1 flex gap-2">
                    <Link href={`/${locale}/admin/service-matrix/${it.id}?service=${selected}`} className="inline-flex items-center justify-center rounded-xl bg-bumn-gradient-primary-11 px-3 py-2 text-sm font-medium text-white shadow-bumn-2 transition hover:opacity-95">{t?.detailButton ?? "Detail / Edit"}</Link>
