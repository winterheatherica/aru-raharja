...
export default function ServiceCertificationCreatePage({ locale, dict }: { locale: Locale; dict?: Dictionary }) {
  ...
  if (!selected) {
    return <main className="mx-auto max-w-7xl p-6 md:pl-72">{t?.serviceRequired ?? "Pilih service dulu di list page."}</main>;
  }
  ...
  return (
    <main className="mx-auto max-w-7xl p-6 grid gap-4 md:pl-72">
      <section className="rounded-2xl border border-bumnslate-10 bg-bumn-gradient-white-4 p-5 shadow-bumn-2 flex items-center justify-between">
        ...
        <Link href={`/${locale}/admin/service-certification?service=${selected}`} className="rounded-xl border border-bumnslate-10 bg-white px-3 py-2 text-sm">{t?.backToList ?? "Back"}</Link>
      </section>
