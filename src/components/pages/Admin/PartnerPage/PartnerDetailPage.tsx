...
export default function PartnerDetailPage({ locale, dict, partnerId }: { locale: Locale; dict?: Dictionary; partnerId: string }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      setError(null);
      try {
        const res = await fetch(`${ADMIN_PARTNER_URL}/${partnerId}`, { credentials: "include" });
        if (!res.ok) throw new Error(await res.text());
        const item: PartnerItem = await res.json();

        setOrderIndex(item.order_index || 0);
        setIsActiveGrid(Boolean(item.is_active_partner_grid));
        setIsActiveScroller(Boolean(item.is_active_partner_scroller));
        setPreviewSrc(partnerImageSrc(item));
    ...
    <main className="mx-auto max-w-7xl p-6 grid gap-4 md:pl-72">
      <section className="rounded-2xl border border-bumnslate-10 bg-bumn-gradient-white-4 p-5 shadow-bumn-2">
        <div className="flex items-center justify-between gap-4">
          <h1 className="text-2xl font-semibold text-bumnblue-2">{t?.titlePrefix ?? "Detail Partner"} • {partnerId}</h1>
          <Link href={`/${locale}/admin/partner`} className="inline-flex items-center justify-center rounded-xl border border-bumnslate-10 bg-white px-3 py-2 text-sm text-bumnslate-6">{t?.backToList ?? "Kembali ke daftar"}</Link>
        </div>
      </section>
      ...
      <form onSubmit={onSave} className="grid gap-3 rounded-2xl border border-bumnslate-10 bg-bumn-gradient-white-4 p-5 shadow-bumn-2">
        ...
        <div className="grid gap-2">
          ...
          <PartnerPreviewCard imageSrc={pickedImagePreview || previewSrc || undefined} alt={current.alt} emptyText={t?.previewEmpty ?? "Belum ada gambar"} />
        </div>
