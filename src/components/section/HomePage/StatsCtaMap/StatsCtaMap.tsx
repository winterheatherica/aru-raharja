...
function StatsCtaMap({ dict, locale }: { dict: Dictionary; locale: Locale }) {
  const s = (dict as any).statsCtaMap ?? {
    metricLabels: {},
  ...
  const metrics = [
    { value: 29, label: s?.metricLabels?.regionalOffices ?? "Kantor Wilayah" },
    { value: 64, label: s?.metricLabels?.branchOffices ?? "Kantor Cabang" },
    { value: 32, label: s?.metricLabels?.serviceOffices ?? "Kantor Pelayanan" },
  ];

  const ctaHref = `/${locale}/complaint`;
  const mapSrc = "/stats-cta-map/indonesia.png";
