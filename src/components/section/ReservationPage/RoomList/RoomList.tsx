...
export function RoomItem({ r, i, roomlist, locale,}: {r: RoomAPI; i: number; roomlist: any; locale: string}) {
  const { ref, visible } = useRevealOnScroll<HTMLDivElement>();

  return (
    <article
      ref={ref}
      className={`flex flex-col sm:flex-row items-stretch gap-4 bg-bumn-gradient-white-4 border border-bumnslate-10 rounded-xl p-4 shadow-bumn-2
...
export default function RoomList({ dict, locale, rooms }: Props) {
  ...
  return (
    <section className="py-8">
      <div
        ref={headerRef}
        className="flex items-center justify-between mb-6 px-2 md:px-16 lg:px-24"
      >
        <h3
          className={`text-xl lg:text-2xl font-semibold text-bumnblue-2
                     transition-all duration-[1000ms] ease-out
                     ${
                       headerVisible
                         ? "opacity-100 translate-x-0"
        ...
        <div
          className={`text-sm text-bumngray-6
                     transition-all duration-[1000ms] ease-out
                     ${
                       headerVisible
                         ? "opacity-100 translate-x-0"
