const about = {
  meta: {
    title: "About Us – PT Aru Raharja",
    description:
      "Learn more about PT Aru Raharja, our vision, mission, and commitment to delivering trusted digital services and solutions.",
  },
  hero: {
    smallTitle: "About Aru Raharja",
    headline:
      "Aru Raharja strives to be a trusted company in providing basic protection against accident risks with the best service.",
    maskingAlt: "masking background",
    buildingAlt: "Aru Raharja building",
  },
  intro:
    "PT Aru Raharja is a State-Owned Enterprise specializing in social insurance, providing basic protection to the public through two (2) coverage programs:",
  list: {
    items: [
      "Passenger Accident Insurance for Public Transportation, based on Law No. 33 of 1964 concerning Mandatory Passenger Accident Insurance Fund.",
      "Legal Liability Insurance for Third Parties, based on Law No. 34 of 1964 concerning Road Traffic Accident Fund.",
    ],
  },
  visionMission: {
    bannerAlt: "Aru Raharja vision banner",
    title: "Vision & Mission",
    visionLabel: "Vision",
    missionLabel: "Mission",
    vision:
      "Becoming a Trusted Company in Providing Basic Protection Against Accident Risks with the Best Service",
    mission:
      "Providing Integrated Digital Basic Protection, Supported by Excellent Human Capital to Strengthen Stakeholder Engagement.",
  },
  history: {
    title: "History",
    intro:
      "Here is the recent journey of Aru Raharja over the last three years.",
    items: [
      {
        year: "2023",
        content: [
          "Year 2023: Aru Raharja continues digital transformation by introducing AI-based claim services to accelerate compensation processes.",
        ],
      },
      {
        year: "2024",
        content: [
          "Year 2024: Aru Raharja expanded partnerships with various government and private institutions to enhance public protection.",
        ],
      },
      {
        year: "2025",
        content: [
          "Year 2025: Aru Raharja launched a new mobile application to provide easier access to compensation services for the public.",
        ],
      },
    ],
  },
  business: {
    title: "Business Fields",
    intro:
      "As stipulated in Article 3, Paragraphs 1 and 2 of the Articles of Association of PT Aru Raharja, the Company's business scope is Social Insurance. The Company's objectives and business activities are to participate in and support the Government's economic, social security, and national development policies, particularly in the field of general insurance. This is achieved through implementing social insurance programs and optimizing the Company's resources to deliver high-quality, competitive services that enhance the Company's value while adhering to the principles of a Limited Liability Company.",
    coreTitle: "Core Business Activities",
    coreItems: [
      "Administering passenger accident insurance for public transportation, in accordance with Law No. 33 of 1964 and its implementing regulations, as well as legal liability insurance for third parties, in accordance with Law No. 34 of 1964 and its implementing regulations.",
      "Entering into and managing motor vehicle insurance agreements and legal liability insurance for third parties in the event of transportation accidents.",
      "Accepting indirect insurance coverage retained by the Company.",
      "Conducting investment activities in compliance with applicable laws and regulations.",
    ],
    extraParagraphs: [
      "In addition to the core business activities, the Company may undertake other business activities to optimize the use of its resources, provided they comply with applicable laws and regulations.",
      "However, with the issuance of Government Regulation of the Republic of Indonesia No. 73 of 1992 concerning the Administration of Insurance Business, particularly Article 33, which states: 'Insurance companies administering Social Insurance are prohibited from administering other insurance programs besides Social Insurance Programs,' and Article 34, which states: 'Insurance companies administering Social Insurance Programs must comply with this Government Regulation and its implementing regulations,' the Company exclusively engages in Social Insurance activities.",
    ],
  },
  culture: {
    title: "Company Culture",
    description:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer fermentum, ante ac pulvinar aliquam, lectus sapien tempor turpis, vel vehicula lacus est vel purus. Donec mollis posuere enim, in auctor nisi commodo vitae. Praesent posuere sollicitudin interdum. Pellentesque eu tempus enim, tincidunt convallis nunc. Quisque ex risus, convallis sed ante a, maximus auctor nunc. Vestibulum lobortis dapibus erat, in lobortis diam mattis vel. Vivamus odio mauris, rhoncus ac purus ut, rutrum tincidunt sem. Nam gravida, ex at ultrices pharetra, tortor lorem efficitur ante, id facilisis ipsum mi sed nulla. Phasellus sit amet imperdiet magna. Fusce sagittis, orci vitae lacinia porta, tellus enim vehicula purus, sit amet fermentum sem nunc ut nulla. Curabitur congue auctor tellus et convallis.",
  },
  subsidiary: {
    title: "Subsidiary",
    name: "Aru Raharja Putera Insurance",
    logoAlt: "Aru Raharja logo",
    description:
      "Founded on 27 November 1993 in Jakarta, it has provided extensive insurance services to people throughout Indonesia for more than two decades.",
  },
  awards: {
    title: "Awards",
    yearLabel: "Year",
    yearAll: "All years",
    year2025: "2025",
    cta: "Certificate of Excellence",
    empty: "No awards found for this year.",
  },
} as const;

export default about;
